...
const UpdateClassPage = () => {
   ...
   useEffect(() => {
      const getClassInfo = async () => {
         setLoading(true);
         try {
            const response = await dispatch(getApiAction(token, id, 'class'))
            console.log('Response:', response);
            messageApi.success('Lấy thông tin lớp thành công!');
            form.setFieldsValue({
               id: response?.id,
               name: response?.name,
            ...
            messageApi.error('Lấy thông tin lớp thất bại!');
         } finally {
            setLoading(false);
         }
      };
   ...
   return (
      <div className="form-container">
         {contextHolder}
         <h1 className="form-title">Cập nhật lớp</h1>
         <Form
            form={form}
            layout="horizontal"
            labelCol={{ span: 6 }}
